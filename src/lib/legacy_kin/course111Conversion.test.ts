...
describe("Course 111 conversion invariants", () => {
  ...
  it("keeps About Automatic Patterning instructional copy and the saved LearnDAK promo as siblings", () => {
    const block = data.lessons
      .flatMap((lesson) => lesson.blocks ?? [])
    ...
    const promo = block!.components![1] as { type: string; legacyComponentId: number; html: string };
    expect(promo.type).toBe("richText");
    expect(promo.legacyComponentId).toBe(9348);
    expect(promo.html).toContain("LearnDesignaKnit.com");
    expect(promo.html).toContain("learndesignaknit.com");
    expect(promo.html).toContain("learn_dak_logos.png");
    ...
    expect(contentItemDisplayTitle(firstNeighbors.prev!.lesson, firstNeighbors.prev!.item)).toBe(
      "Gauge Rulers (Gauge Scales)",
    );
    expect(firstNeighbors.next!.item.legacyComponentId).toBe(9348);

    const promoNeighbors = getCourseContentItemNeighbors(
      course!,
      lesson!.slug,
      aboutItems[1]!.itemSlug,
    );
    expect(contentItemDisplayTitle(promoNeighbors.next!.lesson, promoNeighbors.next!.item)).toBe(
