/**
 * Generated Stitches & Rows SVG for Sleeveless Pullover V-neck Front (straight body).
 *
...
function buildFrame(model: SleevelessFrontStsRowsDiagramModel): { frame: Frame; bands: YBand[] } {
  ...
  // Post-armhole / cross-shoulder width is stitch-true and independent of V depth.
  // Shoulder + neck + shoulder always add back to stitchesAfterArmhole.
...
function drawSilhouette(frame: Frame): string {
  ...
  return [
    ...
    `<path data-role="neckline-outline" d="${polylineD(neckline)}" fill="none" stroke="none"/>`,
...
export function buildSleevelessFrontStsRowsDiagramSvg(
  ...
  const parts = [
    ...
    `<g data-role="v-point" data-x="${fmtNum(frame.cx)}" data-y="${fmtNum(frame.neckStartY)}"></g>`,
  ];
  ...
  const title = "Sleeveless pullover V-neck Front stitches and rows";
