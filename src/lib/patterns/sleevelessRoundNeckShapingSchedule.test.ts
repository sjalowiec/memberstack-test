...
/** Minimal RowEntry factory ù only `row` + `events` matter to the schedule builder. */
...
describe("buildSleevelessRoundNeckShapingSchedule", () => {
  ...
  it("returns null when there is no center bind-off (V-neck / cardigan half front) ù scope protection", () => {
    const timeline = [
  ...
  it("case 1 ù simple neckline with minimal shaping (center + a few singles, no shoulder)", () => {
    const timeline = [
      mkRow(200, [center(10)]),
  ...
  it("case 2 ù initial center bind-off followed by repeated single decreases", () => {
    const timeline = [
      mkRow(300, [center(12)]),
  ...
  it("case 3 ù more than one decrease grouping / interval (steep every row, then gradual every other row)", () => {
    const timeline = [
      mkRow(400, [center(8)]),
  ...
  it("case 4 ù includes real shoulder shaping (outer-edge bind-offs)", () => {
    const timeline = [
      mkRow(500, [center(10)]),
...
describe("shapingScheduleToMapData (adapter)", () => {
  ...
  it("converts row labels to armhole-local RC using firstArmholeRc (single source of truth)", () => {
    ...
    // Center stitches are a stitch count, not an RC ù never shifted.
...
describe("integration ù real Sleeveless Round Neck front timeline", () => {
  ...
  it("map row labels use the SAME armhole-local RC origin as the written instructions/checklist", () => {
    ...
    // The map's lowest label is the center row expressed in armhole-local RC ù not the raw
    // garment RC ù matching formatArmholeLocalRc(garmentRc, armholeStartRow) used elsewhere.
    ...
    // pattern's own debug.frontNecklineStartLocalRC ù proving one shared numbering system.
  ...
  it("case 5 ù row gauge changes the shaping schedule", () => {
