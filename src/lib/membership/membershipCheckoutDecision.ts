...
/**
 * Active free plan ids that can grant member access without Stripe billing
 * (e.g. the "legacy membership" free plan). Plan presence alone is not access;
 * `hasMemberAccess` also requires a valid Watson paid-through date. These are
 * NOT paid membership: they never affect `memberHasActivePaidMembership`,
 * checkout decisions, billing intervals, or pricing.
 */
 ...
 * paid subscription). Plan-presence helper for checkout/display detection — not
 * the access gate. Use `hasMemberAccess` to decide whether the membership is
 * currently valid.
 */
