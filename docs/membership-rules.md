...
- The active Memberstack free plan **legacy membership** (`pln_legacy-membership-t012x0xw0`) can grant the same access as paid membership **only when** Watson `legacy_members.subscriptionexpiring` is today or in the future (America/Los_Angeles calendar day).
- It is part of the shared candidate list (`MEMBER_PLAN_IDS`) consumed by `hasMemberAccess`. Plan presence alone is not enough.
...
- **Expiration is enforced by the access gate.** `hasMemberAccess` requires a valid paid-through date for this plan. Merely having the plan connection or a legacy membership record does not grant access. A scheduled reconciliation still removes expired plan connections from Memberstack; the gate does not wait for that job.

## Legacy annual expiration

Legacy annual members were migrated into Memberstack with the free legacy membership access plan (`pln_legacy-membership-t012x0xw0`) but have **no Stripe subscription**. Their paid-through date lives in Watson (`legacy_members.subscriptionexpiring`). Site access now consults that date at request time (`hasMemberAccess` / `evaluateMemberAccessForRecord`). The scheduled job still removes expired plan connections so Memberstack records stay in sync.
...
- **`hasMemberAccess` now also enforces the paid-through date** for the free legacy plan. Removing the Memberstack connection remains a cleanup step so expired members do not keep a misleading plan on their account.
...
- **The free legacy plan is not sufficient by itself.** Access also requires a unique Watson paid-through date (`legacy_members.subscriptionexpiring`) that is today or in the future (America/Los_Angeles). A legacy record without that valid date does not grant access.
- **Legacy expiration dates do not grant access on their own.** A paid-through date without a qualifying plan does not unlock the site.
...
- For logged-in visitors, client Memberstack is authoritative for current **paid** status. `/membership` and Account status use the same `hasMemberAccess` determination as the access gate: a connected free legacy plan is shown as active / Legacy Access only when Watson `subscriptionexpiring` is today or later. An expired date is shown as ended/expired, not as an active plan. A Watson lookup failure for free-legacy-only members fails closed (wait / could-not-confirm), never Active. A successfully loaded member with **no qualifying access** is purchase-eligible when legacy is `not_found` or expired (not a lookup failure). Server legacy context is consulted when the client has no paid plan and no currently valid free-legacy access.
- A uniquely linked `subscriptionexpiring` that is **today or in the future** without the connected free legacy plan does **not** grant access and must not be described as remaining time or active membership. Display it as an account synchronization issue (`contact_support`) so the customer can reach support before purchasing. Past `subscriptionexpiring` dates and **no legacy record** (`not_found`) recommend purchase. A failed legacy Watson lookup (`lookup_unavailable`) waits / could-not-confirm — it is not treated as “no record,” Expired, or Active. Subscription history may appear in the history accordion only; it never determines current access or current-status labels.
...
- Dynamic Patterns (catalog, builders, saved/generated patterns, print) require **active Knit it Now membership** only — canonical `hasMemberAccess` (paid membership, or free legacy plan with a valid paid-through date). Retired KIN Beta Access does not grant access.
