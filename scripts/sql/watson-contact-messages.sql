...
-- Server-side Postgres only. Public visitors submit through the contact function.
-- RLS enabled with no anon/authenticated policies, so the Data API cannot read or write.
-- Do not use FORCE ROW LEVEL SECURITY (table owner / bypass roles keep server access).
-- Safe to re-run: uses IF NOT EXISTS. Does not modify existing tables.
--
-- Apply against the DEV Watson Postgres database only after explicit approval:
--   psql "$WATSON_DATABASE_URL" -f scripts/sql/watson-contact-messages.sql
--
-- Do not apply this to a shared or production database from an unapproved change.
